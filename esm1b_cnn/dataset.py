...
def crop (pos_var, input,size):
    #If smaller then crop window return whole emb vector + pad w/ zeros to make dim=129x1305
    if (input.shape[0]<=size):
        output = torch.vstack ((input, torch.zeros (size+1-input.shape[0],1305)))
        return output.clone()

    flank = size//2
    #If overflow on right take last 129
    if (pos_var + flank  >= input.shape[0]):
        return input[-(CROP_LEN+1):].clone()
    #Take first 129
    if (pos_var - flank <0):
        return input[:CROP_LEN+1]

    return input [pos_var-flank:pos_var+flank+1].clone()
...
def dataset(INPUT_PATH, esm1b_PATH,out_root):
    ...
    for idx, line in df.iterrows():
        ...
        # combine features
        one_hot = torch.zeros (esm1b.shape[0],len(AA))
        one_hot [pos_var] = b
        out  = torch.hstack ((esm1b,one_hot))

        out = crop (pos_var, out, CROP_LEN)
        assert (out.shape [0]==CROP_LEN+1)
